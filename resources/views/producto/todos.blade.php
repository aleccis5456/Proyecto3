    ...
    <div class="flex items-center justify-center pt-10 bg-gray-100 dark:bg-gray-900"> <!-- Contenedor principal centrado -->
        <div class="max-w-full p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="max-w-full flex justify-between">
                <div>
                    @if (isset($b))
                        Resultados para: <b>{{$b}}</b> <span class="text-gray-500 text-sm">({{count($productos)}})</span>
                    @else
                ...
                <div>
                    <form class="max-w-md mx-auto px-1 pb-1" action="{{ route('producto.busqueda') }}" method="GET" name="form_buscador">
                        ...
                        <tbody>
                            @foreach ($productos as $item)
                                <tr
                                    ...
                                    <th class="px-6 py-4">
                                        <a href=" {{ route('producto.detalle', ['id' => $item->id]) }}" class="hover:text-blue-600">
                                            {{ Str::limit($item->nombre, 37) }}
                                        </a>
                                    </th>
                                    <td class="px-6 py-4 {{ $item->precio_oferta > 0 ? 'text-green-500' : 'text-red-500' }}">
